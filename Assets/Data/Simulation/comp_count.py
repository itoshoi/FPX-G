...
def main():
    ...
    for m in [3]:
        ...
        legend = plot(path_head, axs, 0.07)
        ax_ind += 1
    ...
    plt.subplots_adjust(hspace=0.5, wspace=0.2, top=0.77,
                        bottom=0.14, left=0.1, right=0.9)
    ...
    print(gmodel)
    path_head = f'{gmodel}'
    legend = plot(path_head, ax, 0.07)

    fig.legend(legend[0], legend[1], loc='upper center',
               ncol=6, fontsize=16, mode='expand', markerscale=1.0)
    plt.subplots_adjust(hspace=0.5, wspace=0.2, top=0.80,
                        bottom=0.13, left=0.1, right=0.9)
