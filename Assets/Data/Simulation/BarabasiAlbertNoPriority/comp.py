...
def main():
    ...
    for m in [2, 3, 4, 5, 10]:
        ...
        legend = plot(path_head, axs[ax_ind])
        ax_ind += 1
    ...
    fig, axs = plt.subplots(nrows=1, ncols=9, figsize=(16, 4))
    ax_ind = 0
...
def plot(path_head, ax):
    x = [round(i, 1) for i in np.arange(0.1, 2, 0.1)]
    ...
    for dist in range(2, 4):
        ...
        y = df[df['Distance'] == dist]['OperationCount'].mean()
        ax.plot([x[0], x[-1]], [y, y], label=f'Dist={dist} (2D)',
                linestyle=lstyles[dist-1], color=colors[dist-1], linewidth=1.5,
                marker=None)
