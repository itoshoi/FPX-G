...
def main():
    ...
    for m in [2, 4, 6]:
        ...
        legend = plot(path_head, axs[ax_ind], 800)
        ax_ind += 1
    ...
    plt.savefig('figs/multigoal/barabasi.pdf')
    ...
    plt.subplots_adjust(hspace=0.5, wspace=0.2, top=0.77,
                        bottom=0.15, left=0.05, right=0.98)
    plt.savefig('figs/multigoal/wattsstrogatz.pdf')

    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(16, 4))
    gmodel = 'Tree'
    path_head = f'{gmodel}'
    legend = plot(path_head, ax, 800)

    fig.legend(legend[0], legend[1], loc='upper center',
               ncol=6, fontsize=16, mode='expand', markerscale=1.0)
    plt.subplots_adjust(hspace=0.5, wspace=0.2, top=0.77,
                        bottom=0.15, left=0.05, right=0.98)
    plt.savefig('figs/multigoal/tree.pdf')
...
def plot(path_head, ax, ylim):
    ...
    for dist in range(2, 3):
        ...
        y = df['OperationCount'].mean()
        ax.plot([x[0], x[-1]], [y, y], label='ブラウジング',
